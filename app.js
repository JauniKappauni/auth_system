...
app.post('/forgot-password' , (req, res) => {
    ...
    conn.query('SELECT * FROM users WHERE email = ?', [emailvalue], (err, results) => {
        if(err){
            console.error(err)
            res.send('No email found in the database')
        }
        if(results.length == 0){
            ...
            conn.query('UPDATE users SET reset_token = ?, reset_expires = ? WHERE email = ?', [reset_token, reset_expires, emailvalue], (err2) => {
                if(err){
                    console.error(err2)
                    res.send('No email found in the database')
                }
...
app.get('/reset-password', (req,res) => {
    ...
    if(!reset_token){
        ...
        res.render('reset-password', {title: "Reset Password", token: reset_token})
    }
})
app.post('/reset-password', (req, res) => {
    const reset_token = req.body.token
    const newPassword = req.body.password
    conn.query('SELECT * FROM users WHERE reset_token = ?', [reset_token], (err, results) => {
        if(err){
            console.error(err)
        ...
        if(results.length == 0){
            ...
            const expirationDate = new Date(results[0].token_expires)
                if(expirationDate.getTime() < Date.now()){
                    ...
                    conn.query('UPDATE users SET password = ?, reset_token = NULL, reset_expires = NULL WHERE reset_token = ?', [newPassword, reset_token], (err2) => {
                        if(err2){
                            console.error(err2)
